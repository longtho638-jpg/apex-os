...
const SYMBOLS = [
    ...
    'dogeusdt', 'pepeusdt', 'wifusdt', 'suiusdt', 'nearusdt',
    'aptusdt', 'fetusdt'
];
...
async function fetchKLines(symbol: string, interval: string) {
    try {
        ...
        const res = await fetch(`https://api.binance.com/api/v3/klines?symbol=${symbol.toUpperCase()}&interval=${binanceInterval}&limit=50`); // Increased limit for better accuracy
    ...
    useEffect(() => {
        ...
        binanceWs.onmessage = (event) => {
            ...
            const current = dataRef.current[symbol] || {
                symbol, price, change24h: 0, volume: 0, history: Array(20).fill(price), volumes: Array(20).fill(quantity),
                rsi: 50, upperBand: price * 1.02, lowerBand: price * 0.98, sma20: price, netVolumeDelta: 0, whaleActivity: 'NEUTRAL',
                macroTrend: 'NEUTRAL', macroRsi: 50,
                aiSentiment: 'NEUTRAL', confidenceScore: 50
            };
            ...
            if (newHistory.length > 14) {
                const changes = newHistory.slice(-15).map((p, i, arr) => i === 0 ? 0 : p - arr[i - 1]).slice(1);
            ...
            dataRef.current[symbol] = {
                ...current, price, history: newHistory,
                rsi, // Micro RSI
