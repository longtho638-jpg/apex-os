...
export function useWallet() {
  ...
  const fetchWallet = async () => {
    ...
    try {
      ...
      const totalProfit = closedPositions?.reduce((sum, pos) => sum + (Number(pos.unrealized_pnl) || 0), 0) || 0;

      if (walletData) {
        setWallet({
          total: walletData.balance, // + locked if we track it
          available: walletData.balance,
          locked: 0, // Locked amount requires open order aggregation — compute from orders table when needed
          profit: totalProfit,
          loading: false,
        });
      } else {
  ...
  }, [user, fetchWallet, supabase.channel, supabase.removeChannel]);
