...
import { ArrowRight, Clock, FileJson, History, User, X } from 'lucide-react';
      ...
      <div className="bg-gray-900 border border-white/10 rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">
                History for <span className="text-blue-400">{providerName}</span>
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {loading ? (
            ...
            <div className="text-center py-12 text-gray-500">No audit history found for this provider.</div>
          ) : (
            <div className="space-y-6">
              {logs.map((log) => (
                <div key={log.id} className="relative pl-8 border-l border-gray-700 last:border-0 pb-6 last:pb-0">
                  {/* Timeline dot */}
                  <div
                    className={`absolute -left-2 top-0 w-4 h-4 rounded-full border-2 ${getActionColor(log.action)} bg-gray-900`}
                  ></div>
                  ...
                  <div className="bg-gray-800/50 border border-white/5 rounded-lg p-4 hover:border-white/10 transition-colors">
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-2">
                        <span
                          className={`px-2 py-0.5 text-xs font-bold uppercase rounded ${getActionBadge(log.action)}`}
                        >
                          {log.action}
                        </span>
                    ...
                    {log.action === 'updated' && log.changed_fields && (
                      <div className="space-y-2">
                        {Object.entries(log.changed_fields).map(([key, newVal]) => (
                          <div
                            key={key}
                            className="grid grid-cols-[1fr,auto,1fr] gap-2 items-center text-sm bg-black/20 p-2 rounded"
                          >
                            <div
                              className="text-red-400/80 truncate font-mono text-xs"
                              title={JSON.stringify(log.old_values?.[key])}
                            >
                              {formatValue(log.old_values?.[key])}
                            </div>
                            <ArrowRight className="w-3 h-3 text-gray-600" />
                            <div
                              className="text-emerald-400/80 truncate font-mono text-xs"
