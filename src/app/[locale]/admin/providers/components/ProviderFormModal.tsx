...
export default function ProviderFormModal({
    ...
    const handleTestLink = () => {
        ...
        let url = referralTemplate
            .replace('{locale}', 'en')
            .replace('{partner_uuid}', partnerUuid || 'PLACEHOLDER_UUID');
            ...
            <div className="bg-gray-900 border border-white/10 rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-gradient-to-r from-blue-600/20 to-purple-600/20 shrink-0">
                    <h2 className="text-2xl font-bold text-white">
                        {isEditMode ? '✏️ Edit Provider' : '➕ Add New Provider'}
                    </h2>
                    <button
                        onClick={onClose}
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6 overflow-y-auto flex-1">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className={`w-full px-4 py-2 bg-white/5 border ${errors.provider_code ? 'border-red-500' : 'border-white/10'
                        ...
                        {errors.provider_code && (
                            <p className="text-red-400 text-sm mt-1">{errors.provider_code.message}</p>
                        )}
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            disabled={isSubmitting}
                            className={`w-full px-4 py-2 bg-white/5 border ${errors.provider_name ? 'border-red-500' : 'border-white/10'
                                } rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500`}
                            placeholder="e.g., Binance, OKX, Kraken"
                        />
                        {errors.provider_name && (
                            <p className="text-red-400 text-sm mt-1">{errors.provider_name.message}</p>
                        )}
                    </div>
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 text-xs mt-1">
                            Variables: <code className="bg-white/10 px-1 rounded">{'{locale}'}</code>, <code className="bg-white/10 px-1 rounded">{'{partner_uuid}'}</code>
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            disabled={isSubmitting}
                            rows={4}
                            className={`w-full px-4 py-2 bg-white/5 border ${errors.asset_config ? 'border-red-500' : 'border-white/10'
                        ...
                        />
                        {errors.asset_config && (
                            <p className="text-red-400 text-sm mt-1">{errors.asset_config.message}</p>
                        )}
                    </div>
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            disabled={isSubmitting}
                            rows={3}
                            className={`w-full px-4 py-2 bg-white/5 border ${errors.regulatory_info ? 'border-red-500' : 'border-white/10'
                                } rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono text-sm`}
                            placeholder='{"license_country": "US", "regulatory_body": "SEC"}'
                        />
                        {errors.regulatory_info && (
                            <p className="text-red-400 text-sm mt-1">{errors.regulatory_info.message}</p>
                        )}
                    </div>
                </form>
