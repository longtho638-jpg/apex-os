      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#111] rounded-lg border border-gray-800 p-4 h-[500px] overflow-y-auto space-y-3">
            {loading ? (
              ...
              events.map((e) => (
                <div key={e.id} className="flex gap-3 text-sm border-b border-gray-800 pb-2 last:border-0">
                  <div
                    className={`w-2 h-2 mt-1.5 rounded-full ${e.severity === 'SUCCESS' ? 'bg-green-500' : e.severity === 'WARN' ? 'bg-yellow-500' : 'bg-blue-500'}`}
                  />
                  <div>
            ...
            {!loading && events.length === 0 && <p className="text-center text-gray-500">No signals detected.</p>}
          </div>
        </div>
      </div>
    </div>
