...
import { FileText, Plus, Eye, Edit, Trash2, BarChart } from 'lucide-react';
                        ...
                        <tbody className="divide-y divide-zinc-800">
                            {recentPosts.map((post) => (
                                <tr key={post.id} className="hover:bg-zinc-800/50 transition-colors">
                                    ...
                                    <td className="p-4">
                                        <span className={`px-2 py-1 rounded text-xs font-bold ${post.status === 'Published' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'
                                            }`}>
                                            {post.status}
                                        </span>
                                    </td>
                                    ...
                                    <td className="p-4 text-right space-x-2">
                                        <Button size="sm" variant="ghost" className="h-8 w-8 p-0 hover:text-blue-400"><Edit className="w-4 h-4" /></Button>
                                        <Button size="sm" variant="ghost" className="h-8 w-8 p-0 hover:text-red-400"><Trash2 className="w-4 h-4" /></Button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
