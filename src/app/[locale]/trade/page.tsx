...
import { Activity, ArrowUpRight, ChevronDown, Crown, RefreshCw, TrendingUp } from 'lucide-react';
import { useTranslations } from 'next-intl';
import { useEffect, useState } from 'react';
import { toast } from 'sonner';
import IndicatorPanel, { DEFAULT_INDICATORS, type IndicatorSettings } from '@/components/IndicatorPanel';
...
import { fetchMarketConditions, fetchTradeHistory, triggerSync } from '@/lib/api';
...
export default function TradePage() {
  ...
  // Fee Calculation
  const feeRate = tier === 'SOVEREIGN' ? 0 : tier === 'ARCHITECT' ? 0.02 : tier === 'OPERATOR' ? 0.05 : 0.1;
                ...
                <div className="absolute top-full left-0 mt-2 w-64 bg-[#0a0a0a] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-[100]">
                  {TOP_COINS.map((coin) => (
                    ...
                    >
                      ...
                      {selectedPair.symbol === coin.symbol && <div className="h-2 w-2 rounded-full bg-[#00FF94]" />}
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="flex items-center gap-4">
            ...
            >
              ...
              <span className="text-[10px] font-medium">
                Fee: {feeRate}% {tier === 'SOVEREIGN' ? '(VIP)' : ''}
              </span>
            </div>
          </div>
        </header>
        ...
        <div className="flex-1 p-4 grid grid-cols-12 gap-4 overflow-hidden z-1">
          ...
          <div className="col-span-9 flex flex-col gap-4">
            <div className="flex-1 glass-panel rounded-xl overflow-hidden relative">
              <TradingChart symbol={selectedPair.symbol} basePrice={selectedPair.basePrice} indicators={indicators} />
            </div>
