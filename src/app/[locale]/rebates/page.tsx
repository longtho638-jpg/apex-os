...
import { ArrowDownToLine, Calculator, Coins, RefreshCw, TrendingUp, Wallet } from 'lucide-react';
...
const MOCK_PAYMENT_METHODS = [
  {
    ...
    type: 'crypto_wallet',
    name: 'USDT Wallet',
    details: { address: 'T9x...jK2', network: 'TRC20' },
    is_default: true,
  },
  {
    id: '2',
    type: 'bank_transfer',
    name: 'Vietcombank',
    details: { account_number: '**** 1234', bank_name: 'VCB' },
    is_default: false,
  },
];
...
export default function RebatesPage() {
  ...
  const [paymentMethods, _setPaymentMethods] = useState<any[]>(MOCK_PAYMENT_METHODS);

  // Calculate dynamic rebate rate based on tier
  const rebateRate = tier === 'SOVEREIGN' ? 0.4 : tier === 'ARCHITECT' ? 0.3 : tier === 'OPERATOR' ? 0.2 : 0.1;
  const nextTierRate = tier === 'SOVEREIGN' ? 0.4 : tier === 'ARCHITECT' ? 0.4 : tier === 'OPERATOR' ? 0.3 : 0.2;
              ...
              <GlassCard className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <h2 className="text-4xl font-bold text-white mt-1 tracking-tight">${available.toFixed(2)}</h2>
                  </div>
                  <div className="p-2 rounded-lg bg-blue-500/10">
                    <Wallet className="h-5 w-5 text-blue-400" />
                  </div>
                </div>
                <div className="text-xs text-zinc-500 mt-2">{t('ready_withdraw')}</div>
              </GlassCard>
                  ...
                  <tbody className="text-sm">
                    {(data?.rebate_history || []).slice(0, 5).map((rebate, idx) => (
                      <tr key={idx} className="group border-b border-white/5 hover:bg-white/5 transition-colors">
                        <td className="py-4 pl-4 text-gray-300 group-hover:text-white transition-colors">
                          {rebate.date}
                        </td>
                        <td className="py-4">
