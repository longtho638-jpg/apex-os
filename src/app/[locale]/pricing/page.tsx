...
export default function PricingPage() {
    ...
    const { tier: currentTier } = useUserTier(); // Get real user tier
          ...
          {TIERS.map((tier, index) => (
            ...
            >
              <div className={`
                ...
                <Button3D
                  full
                  variant={currentTier === tier.name.toUpperCase() ? 'glass' : tier.popular ? 'primary' : 'glass'}
                  className={`mb-8 ${currentTier === tier.name.toUpperCase() ? 'opacity-50' : ''}`}
                  onClick={() => currentTier === tier.name.toUpperCase() ? null : router.push('/billing')}
                  disabled={currentTier === tier.name.toUpperCase()}
                >
                  {currentTier === tier.name.toUpperCase() ? 'Current Plan' : tier.cta}
                </Button3D>
