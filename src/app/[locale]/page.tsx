...
import { TIER_ORDER, UNIFIED_TIERS } from '@apex-os/vibe-payment';
import { motion } from 'framer-motion';
import { ArrowRight, Bot, Check, ChevronRight, Coins, Crown, LockKeyhole, Users } from 'lucide-react';
...
export default function Homepage() {
  ...
  const [strategies, setStrategies] = useState<
    { id: string; name: string; author: string; roi: number; winRate: number; followers: number }[]
  >([]);
    ...
    <div className="min-h-screen bg-[#030303] text-white selection:bg-emerald-500/30 overflow-x-hidden font-sans relative">
            ...
            {strategies.length > 0
              ? strategies.map((strat, i) => (
                  ...
                  >
                    ...
                    <Button3D full variant="glass" className="text-sm">
                      {t('marketplace.copy_strategy')}
                    </Button3D>
                  </motion.div>
                ))
              : // Skeleton Loaders
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div>
              ...
              <p className="text-lg text-zinc-400 mb-8 leading-relaxed">
                ...
                {t('viral.description_2')}{' '}
                <span className="text-emerald-400 font-bold">{t('viral.description_revenue')}</span>{' '}
                {t('viral.description_3')}
              </p>

              <ul className="space-y-4 mb-10">
                {[t('viral.benefit_1'), t('viral.benefit_2'), t('viral.benefit_3'), t('viral.benefit_4')].map(
                  (item, i) => (
              ...
              <Button3D onClick={() => router.push('/en/offer')}>{t('viral.cta')}</Button3D>
            </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
            {TIER_ORDER.map((tierId, i) => {
              ...
              return (
                ...
                >
                  {tierId === 'ARCHITECT' && <div className="absolute top-0 left-0 w-full h-1 bg-emerald-500" />}
                  ...
                  <div className="text-xs text-zinc-500 mb-4 p-2 bg-white/5 rounded-lg">
                    ...
                    {' · '}Agents:{' '}
                    <span className="text-white font-bold">{tier.agentSlots === Infinity ? '∞' : tier.agentSlots}</span>
                  </div>
      ...
      <section className="py-24 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10 text-center">
          <h2 className="text-4xl md:text-5xl font-black mb-8">{t('final_cta.title')}</h2>
          <p className="text-xl text-zinc-400 mb-10 max-w-2xl mx-auto">{t('final_cta.description')}</p>
          <Button3D
            onClick={() => router.push('/signup')}
            className="px-12 py-6 text-xl shadow-2xl shadow-emerald-500/30"
          >
            {t('final_cta.cta')}
          </Button3D>
        </div>
