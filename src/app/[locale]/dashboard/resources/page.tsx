...
export default function ResourcesPage() {
    ...
    const handleUnlock = (id: string, price: number, title: string) => {
        ...
        const finalPrice = tier === 'WHALE' ? 0 : price;
                    ...
                    <div className="glass-card rounded-xl p-8 border border-[#00FF94]/20 relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                {[
                                    { id: 'rep-001', title: 'Whale Watch Weekly', price: 50, desc: 'Deep dive into wallet movements >$1M.' },
                                    ...
                                    { id: 'rep-004', title: 'Algo Studio Masterclass', price: 99, desc: '5 Verified Strategies: Scalping, Arb, Grid & DCA.' }
                                ].map((item) => (
                                    <div key={item.id} className="bg-black/40 border border-white/10 rounded-xl p-5 flex flex-col">
                                        ...
                                        >
                                            {unlockedItems.includes(item.id) ? (
                                                ...
                                                <>
                                                    <Lock className="w-4 h-4" /> Unlock ${tier === 'WHALE' ? '0 (VIP)' : item.price}
                                                </>
                                            )}
                                        </button>
                                    </div>
                        ...
                        {filteredCategories.map((category) => (
                            <div key={category.id} className="glass-card rounded-xl p-6 hover:bg-white/5 transition-all duration-300 group">
                                <div className="flex items-center gap-4 mb-6">
                                    ...
                                    <h2 className="text-lg font-bold group-hover:text-[#00FF94] transition-colors">
                                        {category.title}
                                    </h2>
                                </div>
                                <ul className="space-y-3">
                                    {category.items.map((item, idx) => (
                                        <li key={idx}>
                                            <a href={item.link} className="flex items-center justify-between text-zinc-400 hover:text-white transition-colors group/link">
                        ...
                        <div className="space-y-4">
                            {faqs.map((faq) => (
                                <div key={faq.id} className="border border-white/10 rounded-lg overflow-hidden">
                                    ...
                                    {expandedFaq === faq.id && (
                                        <div className="p-4 bg-black/20 text-zinc-400 border-t border-white/10">
                                            {faq.answer}
                                        </div>
                                    )}
                                </div>
