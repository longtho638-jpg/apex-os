        ...
        <div className="max-w-6xl mx-auto space-y-8">
            ...
            >
              ...
              <div className="relative z-10 flex items-start gap-6">
                ...
                <div className="flex-1">
                  <h3 className="text-2xl font-bold text-white mb-2">{t('founders_only_title')}</h3>
                  <p className="text-gray-400 mb-6 max-w-2xl leading-relaxed">{t('founders_only_desc')}</p>
                    ...
                    transactions.map((tx) => (
                      <tr key={tx.id} className="group hover:bg-white/[0.02] transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-mono text-gray-400 group-hover:text-white transition-colors">
                          ...
                          <span className="text-xs text-gray-600">{new Date(tx.created_at).toLocaleTimeString()}</span>
                        </td>
                        ...
                        <td
                          className={cn(
                            ...
                            tx.amount > 0 ? 'text-[#00FF94]' : 'text-red-400',
                          )}
                        >
                          {tx.amount > 0 ? '+' : ''}
                          {tx.amount.toFixed(4)}
          ...
          {isFree && transactions.length > 50 && (
            <div className="text-center p-8 border border-dashed border-white/10 rounded-2xl">
              ...
              <p className="text-sm text-gray-500">Upgrade to Pro to view full transaction history.</p>
            </div>
          )}
        </div>
      </div>
...
function QuickReportCard({ title, description, icon: Icon, format, color, bg, border, onClick, disabled, delay }: any) {
  return (
    ...
    >
      <div
        className={cn(
          'absolute top-0 right-0 w-32 h-32 rounded-full blur-[60px] -mr-16 -mt-16 transition-opacity opacity-0 group-hover:opacity-20',
          bg.replace('/10', ''),
        )}
      />
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[10px] font-bold text-gray-500 border border-white/10 px-2 py-1 rounded bg-black/20">
            {format}
          </span>
        </div>
