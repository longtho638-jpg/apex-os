...
export default function MarketplacePage() {
    ...
    const discount = tier === 'WHALE' ? 0.5 : tier === 'ELITE' ? 0.2 : 0;
    ...
    const handleCopy = async (strat: Strategy) => {
        ...
        try {
            ...
            toast.success(`Subscribed to ${strat.name}`, {
                description: finalPrice > 0 ? `Fee of $${finalPrice.toFixed(2)} deducted (${discount * 100}% OFF).` : 'Free subscription active.'
            });
                    ...
                    <div className="bg-black/80 backdrop-blur-xl rounded-[22px] p-8 flex items-center justify-between border border-white/10">
                        <div>
                            ...
                            <p className="text-zinc-400 max-w-lg mb-6">
                                Consistently outperforming the market with 145% ROI in the last 30 days.
                                Specialized in hunting liquidity clusters on Binance.
                            </p>
                            <div className="flex gap-4">
                                <Button3D variant="primary" className="px-8">Copy Now</Button3D>
                ...
                <div className="flex gap-2 mb-8 overflow-x-auto pb-2">
                    {['ALL', 'HIGH ROI', 'LOW RISK', 'AI POWERED', 'FREE'].map((tab) => (
                        <button
                            ...
                            className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all ${filter === tab
                                ? 'bg-white text-black'
                            ...
                            >
                                <div className="flex justify-between items-start mb-4">
                                    <div className="flex items-center gap-3">
                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${strat.authorTier === 'WHALE' ? 'bg-amber-500 text-black' :
                                            strat.authorTier === 'ELITE' ? 'bg-purple-500 text-white' : 'bg-zinc-700 text-white'
                                            }`}>
                                            {strat.author.substring(0, 2).toUpperCase()}
                                        </div>
                                        <div>
                                            <h3 className="font-bold text-white group-hover:text-[#00FF94] transition-colors">{strat.name}</h3>
                                            <div className="flex items-center gap-1 text-xs text-zinc-400">
                                                <span>by {strat.author}</span>
                                                {strat.authorTier === 'WHALE' && <Crown className="w-3 h-3 text-amber-400" />}
                                            </div>
                                        </div>
                                    </div>
                                    {strat.price === 0 ? (
                                        <Badge variant="secondary" className="bg-emerald-500/10 text-emerald-400 border-emerald-500/20">FREE</Badge>
                                    ) : (
                                ...
                                <p className="text-xs text-zinc-400 mb-6 h-10 line-clamp-2">
                                    {strat.description}
                                </p>
                                ...
                                <div className="mt-auto flex gap-2">
                                    <Button3D full variant="glass" className="text-xs h-9">Details</Button3D>
                                    <Button3D
                                        full
                                        variant="primary"
                                        className="text-xs h-9"
                                        onClick={() => handleCopy(strat)}
                                    >
