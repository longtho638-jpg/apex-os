...
import { Crown, DollarSign, Target, Users, Zap } from 'lucide-react';
...
const AlgoVisualizer = dynamic(
  () => import('@/components/dashboard/AlgoVisualizer').then((mod) => mod.AlgoVisualizer),
  {
    ssr: false,
    loading: () => (
      <div className="h-full w-full bg-white/5 animate-pulse rounded-2xl flex items-center justify-center text-zinc-600 text-xs font-mono">
        Initializing AI...
      </div>
    ),
  },
...
const WhaleWatcherWidget = dynamic(
  () => import('@/components/dashboard/WhaleWatcherWidget').then((mod) => mod.WhaleWatcherWidget),
  {
    ssr: false,
...
const ArbitrageScannerWidget = dynamic(
  () => import('@/components/dashboard/ArbitrageScannerWidget').then((mod) => mod.ArbitrageScannerWidget),
  {
    ssr: false,
    loading: () => <div className="h-full w-full bg-white/5 animate-pulse rounded-2xl" />,
  },
...
const SystemHealthMesh = dynamic(
  () => import('@/components/dashboard/SystemHealthMesh').then((mod) => mod.SystemHealthMesh),
  {
    ssr: false,
    loading: () => <div className="h-full w-full bg-white/5 animate-pulse rounded-2xl" />,
  },
        ...
        <div className="flex flex-col md:grid md:grid-cols-12 md:grid-rows-12 gap-4 md:h-full">
          ...
          <div className="md:col-span-12 md:row-span-2 grid grid-cols-2 md:grid-cols-4 gap-4 shrink-0">
            ...
            <MetricCard
              ...
              label={t('metrics.active_referrals')}
          ...
          <div className="md:col-span-4 md:row-span-10 flex flex-col md:grid md:grid-rows-3 gap-4 min-h-[400px] md:min-h-0">
            ...
            <div className="md:row-span-1 grid grid-cols-2 gap-4 h-24 md:h-auto">
              <Link
                ...
                className="bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 rounded-2xl border border-emerald-500/20 p-4 flex flex-col justify-center items-center hover:bg-emerald-500/20 transition-all cursor-pointer group relative overflow-hidden"
              >
                <div className="absolute inset-0 bg-emerald-500/5 opacity-0 group-hover:opacity-100 transition-opacity" />
                <Zap className="w-8 h-8 text-emerald-400 mb-2 group-hover:scale-110 transition-transform relative z-10" />
                <span className="text-xs font-bold text-emerald-100 relative z-10">
                  {t('quick_actions.ai_trading_signals')}
                </span>
              </Link>
              <div className="bg-white/5 rounded-2xl border border-white/10 overflow-hidden relative flex flex-col">
                ...
                <div className="absolute bottom-2 w-full text-center text-[10px] text-zinc-500 font-mono">
                  SYSTEM ONLINE
                </div>
              </div>
            </div>
    ...
    >
      ...
      <div>
        ...
        {loading ? (
          ...
          <p
            className={`text-2xl font-bold tracking-tight ${customDisplay === 'CRITICAL' ? 'text-red-500' : customDisplay === 'WARNING' ? 'text-yellow-500' : ''}`}
          >
            {customDisplay ? (
              customDisplay
            ) : (
