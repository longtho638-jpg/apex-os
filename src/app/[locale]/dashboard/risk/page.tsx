...
import { Shield, Sliders, AlertTriangle } from 'lucide-react';
        ...
        <Card className="border-white/10 bg-black/40">
            ...
            <CardContent>
                <div className="flex items-center justify-between mb-4">
                    ...
                    <span className={`font-bold px-3 py-1 rounded-full text-xs border ${status === 'HEALTHY' ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' :
                ...
                <div className="space-y-2">
                    <div className="flex justify-between text-xs text-zinc-500">
                        ...
                        <span>${Math.abs(dailyLoss).toFixed(2)} / ${maxDrawdown}</span>
                    </div>
                    <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden">
                        <div
                            className={`h-full transition-all duration-500 ${status === 'CRITICAL' ? 'bg-red-500' :
    ...
    return (
        <Card className="border-white/10 bg-black/40">
            ...
            <CardContent className="space-y-6">
                ...
                {/* Emergency Toggle */}
                <div className={`p-4 rounded-lg border flex items-start gap-3 transition-colors ${emergencyLock ? 'bg-red-500/10 border-red-500/30' : 'bg-yellow-500/5 border-yellow-500/10'}`}>
                    <AlertTriangle className={`w-5 h-5 shrink-0 ${emergencyLock ? 'text-red-500 animate-pulse' : 'text-yellow-500'}`} />
                    <div className="text-xs text-zinc-400 flex-1">
                        ...
                        {emergencyLock ? 'Trading suspended. Contact support to unlock.' : 'Prevent new orders if market volatility exceeds 10%.'}
                    </div>
                    <Switch
                ...
                <main className="flex-1 flex items-center justify-center relative overflow-hidden">
                    ...
                    <GlassCard className="p-12 max-w-lg text-center z-10 border-red-500/30">
                        ...
                        <p className="text-zinc-400 mb-8">
                            Automated risk protection is exclusive to PRO and WHALE tiers.
                        </p>
                        <button
