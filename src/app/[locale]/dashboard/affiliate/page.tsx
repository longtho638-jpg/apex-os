...
import { Crown, Globe, Lock, QrCode, Rocket, TrendingUp, Wallet, Zap } from 'lucide-react';
import { useEffect, useState } from 'react';
import { toast } from 'sonner';
import { ViralNetwork } from '@/components/affiliate/ViralNetwork';
...
export default function AffiliateDashboard() {
  ...
  const viralStats = [
    { level: 1, users: stats?.total_referrals || 0, volume: 450000, commission: 0.2, earnings: 450, locked: false },
    { level: 2, users: 45, volume: 1200000, commission: 0.1, earnings: 1200, locked: false },
    ...
    <div className="h-full overflow-y-auto custom-scrollbar p-6 bg-[#030303] text-white font-sans">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div className="relative z-10 flex justify-between items-start">
              <div>
                ...
                <div className="flex gap-4 mt-3">
                  <p className="text-emerald-400 text-sm flex items-center gap-1 font-bold">
                    <TrendingUp className="w-4 h-4" /> +$450.00 this week
                  </p>
                  <p className="text-zinc-500 text-sm flex items-center gap-1">
                    <Wallet className="w-4 h-4" /> Lifetime: ${totalEarned.toLocaleString()}
                  </p>
                </div>
              </div>
              <div className="p-3 rounded-2xl bg-[#00FF94]/10 border border-[#00FF94]/20">
          ...
          <div className="rounded-3xl border border-white/10 bg-white/5 p-8 flex flex-col justify-between relative overflow-hidden group">
            ...
            <div>
              ...
              <div className="text-4xl font-black mb-1 text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400">
                ELITE
              </div>
            ...
            <div className="mt-6">
              <div className="flex justify-between text-xs mb-2">
                <span className="text-zinc-400">Next Tier Progress</span>
      ...
      <div className="flex items-center justify-between mb-8 bg-white/5 border border-white/10 rounded-2xl p-2 pl-6">
        ...
        <div className="flex gap-2 shrink-0">
          <Button3D onClick={handleCopy} variant="glass" className="h-10 px-4 py-2">
            {copied ? 'Copied!' : 'Copy Link'}
          </Button3D>
          <Button3D variant="glass" className="w-10 h-10 p-0 flex items-center justify-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {viralStats.map((stat) => (
          <div
            key={stat.level}
            className={`p-5 rounded-2xl border relative overflow-hidden group transition-all ${stat.locked ? 'bg-red-900/5 border-red-500/20 opacity-80' : 'bg-white/5 border-white/10 hover:bg-white/[0.07]'}`}
          >
            {stat.locked && (
            ...
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-2">
                <Globe className={`w-4 h-4 ${stat.locked ? 'text-zinc-600' : 'text-blue-400'}`} />
                <span className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Level {stat.level}</span>
              </div>
              <Badge
                variant={stat.locked ? 'destructive' : 'secondary'}
                className="bg-white/10 hover:bg-white/20 text-white border-0 font-mono"
              >
                {stat.commission * 100}%
              </Badge>
            </div>
            <div className="space-y-1 relative z-10">
              <div className="text-2xl font-bold text-white flex items-baseline gap-1">
                {stat.users} <span className="text-xs font-normal text-zinc-500">active</span>
              </div>
              <div className="text-xs text-zinc-400 font-mono">Vol: ${stat.volume.toLocaleString()}</div>
            </div>
            <div className="mt-4 pt-3 border-t border-white/5 flex justify-between items-center relative z-10">
              ...
              <span className={`font-mono font-bold ${stat.locked ? 'text-red-400' : 'text-[#00FF94]'}`}>
