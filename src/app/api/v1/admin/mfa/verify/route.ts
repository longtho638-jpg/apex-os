...
export async function POST(request: NextRequest) {
  try {
    ...
    if (tempTokenEmail && tempTokenEmail !== email) {
      return NextResponse.json({ error: 'Email mismatch. Please login again.' }, { status: 401 });
    }
    ...
    try {
      if (useRecoveryCode) {
        ...
        const result = await verifyRecoveryCode(token, admin.mfa_recovery_codes || []);
    ...
    try {
      ...
      if (!admin.mfa_enabled) {
        await supabase.from('admin_users').update({ mfa_enabled: true }).eq('email', email);
      }
    } catch (dbError: any) {
    ...
    try {
      ...
      return NextResponse.json({ error: `JWT error: ${jwtError.message}` }, { status: 500 });
    }
  } catch (error: any) {
