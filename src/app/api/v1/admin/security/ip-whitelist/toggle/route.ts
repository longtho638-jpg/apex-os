...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!adminId || typeof enabled !== 'boolean') {
      return NextResponse.json({ error: 'Admin ID and enabled flag are required' }, { status: 400 });
    }

    // Update IP whitelist enabled status
    const { error } = await supabase.from('admin_users').update({ ip_whitelist_enabled: enabled }).eq('id', adminId);
    ...
    return NextResponse.json({
      success: true,
      enabled,
      message: enabled ? 'IP whitelisting enabled' : 'IP whitelisting disabled',
    });
  } catch (error) {
