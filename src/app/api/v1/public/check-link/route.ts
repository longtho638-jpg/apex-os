...
export async function POST(request: NextRequest) {
  try {
    ...
    // 7. Exchange API verification not yet implemented — returns mock response until exchange SDK is integrated
    return NextResponse.json({
      success: true,
      is_linked: false,
 ...
 */
async function generateReferralLink(supabaseAdmin: any, exchange: string, locale: string = 'en'): Promise<string> {
  try {
    // Get provider config
    ...
    if (config.referral_link_template) {
      return config.referral_link_template.replace('{partner_uuid}', config.partner_uuid).replace('{locale}', locale);
    }
