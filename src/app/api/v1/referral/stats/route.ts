...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!userId) {
      ...
      const supabaseToken = request.cookies.get('sb-access-token')?.value || request.cookies.get(sbTokenName)?.value;
    ...
    const [userInfoRes, commissionsRes, referralsRes] = await Promise.all([
      ...
      supabase.from('commissions').select('amount, created_at').eq('referrer_id', userId),

      // 3. Get Referrals List
      supabase.from('users').select('id, created_at, email').eq('referred_by', userId),
    ]);
    ...
    const thisMonthCommission =
      commissions?.filter((c) => c.created_at >= startOfMonth).reduce((sum, c) => sum + Number(c.amount), 0) || 0;

    const formattedReferrals =
      referrals?.map((ref) => ({
        referee_id: ref.id,
        signup_date: ref.created_at,
        volume: 0, // Volume aggregation from orders table pending (join orders on referee_id)
        commission: 0, // Commission aggregation per user pending (join commissions on referee_id)
        status: 'active',
      })) || [];
