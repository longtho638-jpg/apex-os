...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [userRes, walletRes, virtualWalletRes, positionsRes, tradesRes] = await Promise.all([
      supabase.from('users').select('id, email, full_name, subscription_tier, monthly_volume, created_at').eq('id', id).single(),
      supabase.from('wallets').select('balance_usd, reserved_balance, total_earned, total_withdrawn').eq('user_id', id).single(),
      supabase.from('virtual_wallets').select('balance, initial_balance, total_pnl').eq('user_id', id).single(),
      supabase.from('virtual_positions').select('id, symbol, side, size, entry_price, current_price, pnl, status').eq('user_id', id).eq('status', 'OPEN'),
      supabase.from('virtual_positions').select('id, symbol, side, size, entry_price, exit_price, pnl, closed_at').eq('user_id', id).order('closed_at', { ascending: false }).limit(10)
    ]);
