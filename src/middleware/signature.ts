...
export async function validateRequestSignature(request: NextRequest) {
  ...
  const CRITICAL_PATHS = ['/api/v1/admin/finance', '/api/v1/finance', '/api/v1/trade/execute'];
  ...
  if (!isValid) {
    return NextResponse.json({ success: false, message: 'Invalid request signature' }, { status: 403 });
  }
