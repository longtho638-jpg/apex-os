...
import type { Signal } from './AlphaDashboard';
...
export const SignalCard = memo(({ signal, ticker, isSelected, onClick, whaleScore, techScore }: SignalCardProps) => {
  ...
  const _pulseIntensity = hybridScore > 70 ? 'animate-pulse-fast' : 'animate-pulse-slow';
    ...
    >
      ...
      <div className="flex justify-between items-end relative z-10">
        ...
        <div className="w-16 h-8 opacity-80 group-hover:opacity-100 transition-opacity">
          <MiniSparkline data={ticker?.history || []} color={isBuy ? '#10B981' : isSell ? '#EF4444' : '#71717a'} />
        </div>
      </div>
