...
import { type TierId, UNIFIED_TIERS } from '@apex-os/vibe-payment';
      ...
      <Card className="p-6 bg-gradient-to-br from-emerald-500/10 to-blue-500/10 border-emerald-500/20">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-zinc-400 mt-1">{t('current_structure')}</p>
          </div>
          <div className="text-right">
            <div className="text-3xl font-bold text-emerald-400">{(rates.total * 100).toFixed(0)}%</div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
          <div className="bg-black/40 p-4 rounded-lg border border-white/5">
            ...
            <div className="text-2xl font-bold text-white">{(rates.l1 * 100).toFixed(0)}%</div>
          </div>
          <div className="bg-black/40 p-4 rounded-lg border border-white/5">
            <div className="text-xs text-zinc-500 mb-1">{t('level_2')}</div>
            <div className="text-2xl font-bold text-white">{(rates.l2 * 100).toFixed(0)}%</div>
          </div>
          <div className="bg-black/40 p-4 rounded-lg border border-white/5">
            <div className="text-xs text-zinc-500 mb-1">{t('level_3')}</div>
            <div className="text-2xl font-bold text-white">{(rates.l3 * 100).toFixed(0)}%</div>
          </div>
          <div className="bg-black/40 p-4 rounded-lg border border-white/5">
            <div className="text-xs text-zinc-500 mb-1">{t('level_4')}</div>
            <div className="text-2xl font-bold text-white">{(rates.l4 * 100).toFixed(0)}%</div>
          </div>
        </div>
      </Card>
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-4 text-sm">
                ...
                {currentTier === 'OPERATOR' && (
                  <>
                    <div>
                      <span className="text-zinc-500">{t('current')}</span> <span className="font-bold">20%</span>
                    </div>
                    <div>
                ...
                {currentTier === 'ARCHITECT' && (
                  <>
                    <div>
                      <span className="text-zinc-500">{t('current')}</span> <span className="font-bold">30%</span>
                    </div>
                    <div>
