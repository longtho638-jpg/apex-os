...
import { Activity, Brain, Target, TrendingDown, TrendingUp, Zap } from 'lucide-react';
import type { FeatureImportance, MLPrediction } from '@/lib/ml/types';
...
export function MLPredictionPanel({ prediction, featureImportance, loading }: MLPredictionPanelProps) {
  ...
  const predictionColor =
    predictedClass === 'BUY' ? 'text-green-400' : predictedClass === 'SELL' ? 'text-red-400' : 'text-yellow-400';

  const predictionBg =
    predictedClass === 'BUY' ? 'bg-green-500/20' : predictedClass === 'SELL' ? 'bg-red-500/20' : 'bg-yellow-500/20';
    ...
    >
      ...
      <div className="relative z-10">
        <div className="flex justify-between items-center mb-3">
          ...
          <span className={`text-2xl font-bold font-mono ${predictionColor}`}>{(confidence * 100).toFixed(0)}%</span>
        </div>
              ...
              >
                <div className="flex-1">
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="text-purple-400 font-mono">{(feature.importance * 100).toFixed(0)}%</span>
                  </div>
