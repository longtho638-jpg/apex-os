...
import type { Exchange, VerificationResult } from '@/types/exchange';
...
export default function ConnectExchange() {
  const _t = useTranslations('DashboardComponents.ConnectExchange');
  const [selectedExchange, setSelectedExchange] = useState<Exchange | null>(null);
  const [viewMode, setViewMode] = useState<'select' | 'action' | 'verify'>('select');
  const [uid, setUid] = useState('');
  const [status, setStatus] = useState<'idle' | 'verifying' | 'success' | 'error'>('idle');
  const [message, setMessage] = useState('');
  const [_resultData, setResultData] = useState<VerificationResult | null>(null);
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
          {SUPPORTED_EXCHANGES.map((ex) => (
            ...
            >
              ...
              <span className="capitalize text-[10px] font-mono font-bold text-gray-400 group-hover/btn:text-[#00FF00]">
                {ex}
              </span>
            </button>
          ))}
                ...
                {viewMode === 'action' && (
                  <div className="space-y-4">
                    <p className="text-sm text-zinc-300 mb-4 text-center">
                      Do you already have a {selectedExchange} account?
                    </p>
                ...
                {viewMode === 'verify' && (
                  <div className="space-y-6">
                    <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl text-xs text-blue-200 space-y-2">
                      ...
                      <p>
                        Most exchanges allow you to create a <strong>Sub-Account</strong> to trade separately. This
                        keeps your main account safe and allows you to link just this sub-account to Apex for rebates.
                      </p>
                      <div className="flex gap-2 pt-2">
                        ...
                        <button
                          onClick={copyRefCode}
                          className="px-3 py-1.5 bg-blue-900/30 hover:bg-blue-900/50 text-blue-300 rounded-md border border-blue-500/30 flex items-center gap-2 transition-colors"
                        >
