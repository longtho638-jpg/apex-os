...
export function QuantIndicatorsPanel({ ticker, symbol }: QuantPanelProps) {
    ...
    const bbPercentage = upperBand && lowerBand
        ? ((price - lowerBand) / (upperBand - lowerBand)) * 100
        : 50;
    ...
    return (
        ...
        >
            ...
            <div className="absolute top-0 left-0 w-full h-full opacity-30 pointer-events-none">
                <div className={`absolute top-0 left-0 w-32 h-32 rounded-full blur-3xl ${isOversold ? 'bg-green-500/30' : isOverbought ? 'bg-red-500/30' : 'bg-purple-500/20'
                    }`} />
            </div>
            ...
            <div className="flex items-center justify-between mb-2 relative z-10">
                ...
                <div className={`px-2 py-0.5 rounded text-[10px] font-bold ${macroTrend === 'BULLISH' ? 'bg-green-500/20 text-green-400' :
                    macroTrend === 'BEARISH' ? 'bg-red-500/20 text-red-400' :
            ...
            <div className="space-y-2 relative z-10">
                ...
                <div className="space-y-1.5">
                    <div className="flex items-center justify-between text-xs">
                        ...
                        <span className={`font-mono font-bold ${isOversold ? 'text-green-400' : isOverbought ? 'text-red-400' : 'text-white'
                            }`}>
                    ...
                    <div className="relative h-2 bg-black/30 rounded-full overflow-hidden">
                        ...
                        <motion.div
                            className={`absolute top-0 left-0 h-full rounded-full ${isOversold ? 'bg-green-500' : isOverbought ? 'bg-red-500' : 'bg-purple-500'
                                }`}
                            initial={{ width: 0 }}
                ...
                <div className="space-y-1.5">
                    <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="text-white font-mono font-bold">
                            {bbPercentage.toFixed(0)}%
                        </span>
                    </div>
                    <div className="relative h-2 bg-black/30 rounded-full overflow-hidden">
                        <motion.div
                            className={`absolute top-0 left-0 h-full rounded-full ${bbPercentage < 20 ? 'bg-green-500' :
                                bbPercentage > 80 ? 'bg-red-500' :
                                    'bg-blue-500'
                                }`}
                            initial={{ width: 0 }}
                ...
                <div className="grid grid-cols-2 gap-2 pt-2 border-t border-white/5">
                    <div className="bg-black/20 rounded p-2">
                        ...
                        <div className={`text-sm font-bold font-mono ${(rsi || 50) < 30 ? 'text-green-400' : (rsi || 50) > 70 ? 'text-red-400' : 'text-white'
                            }`}>
                            {(rsi || 50).toFixed(1)}
                        </div>
                    </div>
                    <div className="bg-black/20 rounded p-2">
                        ...
                        <div className={`text-sm font-bold font-mono ${(macroRsi || 50) < 30 ? 'text-green-400' : (macroRsi || 50) > 70 ? 'text-red-400' : 'text-white'
                            }`}>
                ...
                <div className="flex flex-wrap gap-1.5 pt-2">
                    {isOversold && (
                        <div className="px-2 py-0.5 bg-green-500/20 text-green-400 text-[9px] font-bold rounded">
                            OVERSOLD
                        </div>
                    )}
                    {isOverbought && (
                        <div className="px-2 py-0.5 bg-red-500/20 text-red-400 text-[9px] font-bold rounded">
                            OVERBOUGHT
                        </div>
                    )}
                    {hasVolume && (
                        <div className="px-2 py-0.5 bg-blue-500/20 text-blue-400 text-[9px] font-bold rounded">
                            HIGH VOLUME
                        </div>
                    )}
                    {bbPercentage < 20 && (
                        <div className="px-2 py-0.5 bg-purple-500/20 text-purple-400 text-[9px] font-bold rounded">
                            LOWER BAND
                        </div>
                    )}
                    {bbPercentage > 80 && (
                        <div className="px-2 py-0.5 bg-orange-500/20 text-orange-400 text-[9px] font-bold rounded">
                            UPPER BAND
                        </div>
                    )}
                </div>
