...
import { Activity, BarChart2, Crosshair, Shield, X, Zap } from 'lucide-react';
          ...
          <div className="lg:col-span-1 bg-[#050505] flex flex-col overflow-hidden">
            <div className="flex-1 overflow-y-auto custom-scrollbar p-4 space-y-6">
              ...
              <div className="p-4 bg-purple-900/10 border border-purple-500/20 rounded-xl">
                ...
                <p className="text-xs text-purple-100/80 leading-relaxed font-medium">{insight}</p>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span
                    className={`text-xs font-mono font-bold ${netVolumeDelta > 0 ? 'text-emerald-400' : 'text-red-400'}`}
                  >
                    {netVolumeDelta > 0 ? '+' : ''}
                    {(netVolumeDelta / 1000).toFixed(1)}k
                  </span>
                </div>
                <div className="h-1.5 w-full bg-zinc-800 rounded-full overflow-hidden flex">
                  <div className="w-1/2 flex justify-end border-r border-zinc-700">
                    {netVolumeDelta < 0 && (
                      <div
                        className="h-full bg-red-500 rounded-l-sm"
                        style={{ width: `${Math.min(Math.abs(netVolumeDelta) / 5000, 100)}%` }}
                      />
                    )}
                  </div>
              ...
              <div className="flex justify-between text-xs mb-3 bg-white/5 p-2 rounded-lg border border-white/5">
                ...
                <div className="text-right">
                  ...
                  <span className="text-[#00FF94] font-mono font-bold">
                    ${requiredMargin.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                  </span>
                </div>
              </div>

              <Button3D full variant={isBuy ? 'primary' : 'danger'} className="shadow-xl py-3">
                {' '}
