...
import { Activity, AlertTriangle, Shield, Target, TrendingDown, Zap } from 'lucide-react';
...
export function RiskDashboard({ metrics, portfolioValue, isDemo = false }: RiskDashboardProps) {
  ...
  const { var95, cvar95, sharpeRatio, sortinoRatio, maxDrawdown, currentDrawdown, volatility } = metrics;

  // Risk levels
  const varLevel = var95 > 0.03 ? 'HIGH' : var95 > 0.02 ? 'MEDIUM' : 'LOW';
  const sharpeLevel = sharpeRatio < 0.5 ? 'LOW' : sharpeRatio < 1.5 ? 'MEDIUM' : 'HIGH';
  const ddLevel = maxDrawdown > 0.15 ? 'HIGH' : maxDrawdown > 0.1 ? 'MEDIUM' : 'LOW';
    ...
    >
        ...
        <div className="flex justify-between text-xs mb-2">
          ...
          <span
            className={`font-bold font-mono ${
              varLevel === 'HIGH' ? 'text-red-400' : varLevel === 'MEDIUM' ? 'text-yellow-400' : 'text-green-400'
            }`}
          >
            {(var95 * 100).toFixed(2)}%
          </span>
      ...
      <div className="grid grid-cols-2 gap-3 relative z-10">
        ...
        <div className="bg-black/20 rounded-lg p-3 border border-white/5">
          ...
          <div className="text-lg font-bold text-orange-400 font-mono">{(cvar95 * 100).toFixed(2)}%</div>
          <div className="text-[9px] text-zinc-600 mt-1">Expected tail loss</div>
        </div>
        ...
        <div className="bg-black/20 rounded-lg p-3 border border-white/5">
          ...
          <div
            className={`text-lg font-bold font-mono ${
              sharpeLevel === 'HIGH' ? 'text-green-400' : sharpeLevel === 'MEDIUM' ? 'text-yellow-400' : 'text-red-400'
            }`}
          >
            {sharpeRatio.toFixed(2)}
          </div>
          <div className="text-[9px] text-zinc-600 mt-1">Risk-adjusted return</div>
        </div>
        ...
        <div className="bg-black/20 rounded-lg p-3 border border-white/5">
          ...
          <div className="text-lg font-bold text-purple-400 font-mono">{sortinoRatio.toFixed(2)}</div>
          <div className="text-[9px] text-zinc-600 mt-1">Downside-adjusted</div>
        </div>
        ...
        <div className="bg-black/20 rounded-lg p-3 border border-white/5">
          ...
          <div
            className={`text-lg font-bold font-mono ${
              ddLevel === 'HIGH' ? 'text-red-400' : ddLevel === 'MEDIUM' ? 'text-yellow-400' : 'text-green-400'
            }`}
          >
            {(maxDrawdown * 100).toFixed(1)}%
          </div>
          <div className="text-[9px] text-zinc-600 mt-1">Peak-to-trough</div>
        </div>
      </div>
      ...
      <div className="relative z-10">
        <div className="flex justify-between text-xs mb-2">
          ...
          <span className="text-white font-mono font-bold">{(currentDrawdown * 100).toFixed(2)}%</span>
        </div>
        <div className="relative h-3 bg-black/30 rounded-full overflow-hidden">
          <motion.div
            className="absolute top-0 left-0 h-full bg-gradient-to-r from-yellow-500 via-orange-500 to-red-500"
            initial={{ width: 0 }}
            animate={{ width: `${Math.min(currentDrawdown / 0.2, 1) * 100}%` }}
            transition={{ type: 'spring', stiffness: 100 }}
          />
          {/* Warning line at 10% */}
      ...
      <div className="relative z-10 bg-black/20 rounded-lg p-3 border border-white/5">
        <div className="flex justify-between items-center">
          <div>
            ...
            <div className="text-xl font-bold text-blue-400 font-mono">{(volatility * 100).toFixed(1)}%</div>
          </div>
          <div className="text-right">
            ...
            >
              {volatility > 0.4 ? 'HIGH' : volatility > 0.25 ? 'MODERATE' : 'LOW'}
            </div>
          </div>
        </div>
      </div>
