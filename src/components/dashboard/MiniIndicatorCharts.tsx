...
export function MiniIndicatorCharts({ ticker }: MiniIndicatorChartsProps) {
    ...
    const validRsi = typeof rsi === 'number' && !isNaN(rsi) ? rsi : 50;
    const validMacroRsi = typeof macroRsi === 'number' && !isNaN(macroRsi) ? macroRsi : 50;
        ...
        <div className="grid grid-cols-2 gap-2 p-2 bg-black/20 border-t border-white/10">
            ...
            >
                <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-sm font-mono font-bold ${effectiveRsi < 30 ? 'text-green-400' :
                        effectiveRsi > 70 ? 'text-red-400' :
                            'text-white'
                        }`}>
                ...
                <div className="relative h-12 bg-black/30 rounded overflow-hidden">
                    ...
                    {/* RSI Level Indicator */}
                    <motion.div
                        className={`absolute left-0 right-0 h-1 ${effectiveRsi < 30 ? 'bg-green-500' :
                                effectiveRsi > 70 ? 'bg-red-500' :
                                    'bg-blue-500'
                            }`}
                ...
                <div className="mt-1 text-center">
                    {effectiveRsi < 30 && (
                        <span className="text-[8px] text-green-400 font-bold">OVERSOLD</span>
                    )}
                    {effectiveRsi > 70 && (
                        <span className="text-[8px] text-red-400 font-bold">OVERBOUGHT</span>
                    )}
