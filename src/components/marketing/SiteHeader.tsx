...
export function SiteHeader() {
  ...
  return (
    <>
      <header
        className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-[#030303]/80 backdrop-blur-xl border-b border-white/5 py-3' : 'bg-transparent py-6'}`}
      >
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 cursor-pointer group" onClick={() => router.push('/')}>
              ...
              <span className="text-xl font-bold tracking-tight">
                Apex
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-cyan-400">OS</span>
              </span>
            </div>
      ...
      {mobileMenuOpen && (
        ...
        >
          <div className="flex flex-col gap-6">
            ...
            <button onClick={() => router.push('/login')} className="text-xl font-bold text-zinc-400 text-left">
              {t('nav.signin')}
            </button>
            <button onClick={() => router.push('/signup')} className="text-xl font-bold text-emerald-400 text-left">
              {t('nav.get_started')}
            </button>
          </div>
        </motion.div>
