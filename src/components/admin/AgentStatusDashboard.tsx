  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {loading ? (
          <div className="col-span-2 text-center text-gray-500 py-8">Loading agent status...</div>
        ) : agents.length === 0 ? (
          ...
          agents.map((agent) => (
            <div key={agent.id} className="bg-[#111111] border border-white/10 rounded-xl p-6 space-y-4">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between text-gray-400">
                  ...
                  <span className="text-white">
                    {agent.minutesSinceLastCheck < 1 ? 'Just now' : `${agent.minutesSinceLastCheck}m ago`}
                  </span>
                </div>
                {agent.error_message && (
