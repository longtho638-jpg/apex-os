...
import { Activity, AlertTriangle, Lock, ShieldAlert } from 'lucide-react';
...
export default function RiskManagement() {
  ...
  return (
    <div className="space-y-8">
      {/* Kill Switch Section */}
      <Card
        className={`border-2 transition-all duration-500 ${killSwitchActive ? 'bg-red-950/30 border-red-500 shadow-[0_0_50px_rgba(220,38,38,0.2)]' : 'bg-black border-gray-800'}`}
      >
        <CardContent className="p-8 flex flex-col items-center text-center">
          <div
            className={`w-24 h-24 rounded-full flex items-center justify-center mb-6 transition-all duration-500 ${killSwitchActive ? 'bg-red-500 animate-pulse shadow-[0_0_30px_#ef4444]' : 'bg-gray-900 border border-gray-800'}`}
          >
          ...
          <p className="text-gray-400 mb-8 max-w-md">{t('kill_switch.description')}</p>

          <Button
            size="lg"
            onClick={() => setKillSwitchActive(!killSwitchActive)}
            className={`h-16 px-12 text-lg font-bold tracking-widest transition-all duration-300 ${
              killSwitchActive
