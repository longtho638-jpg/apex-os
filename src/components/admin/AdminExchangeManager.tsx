...
import { AlertCircle, CheckCircle2, Key, Loader2, Lock, Network, Save, Settings, X } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {EXCHANGES.map((exchange) => {
            ...
            return (
              <Card
                key={exchange}
                className={`bg-[#111111] border ${isConfigured ? 'border-[#00FF94]/20' : 'border-gray-800'} hover:border-[#00FF94]/50 transition-all`}
              >
                <CardContent className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-10 h-10 rounded-lg flex items-center justify-center font-bold text-sm ${isConfigured ? 'bg-[#00FF94]/10 text-[#00FF94]' : 'bg-gray-800 text-gray-400'}`}
                      >
          ...
          <Card className="w-full max-w-md bg-[#111111] border-gray-800">
            ...
            <CardContent className="p-6 space-y-4">
              <div className="p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg flex items-start gap-3">
                ...
                <p className="text-xs text-blue-300 leading-relaxed">{t('secure_storage_notice')}</p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    setFormData({ ...formData, partner_uuid: e.target.value })
                  }
