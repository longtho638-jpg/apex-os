...
import { DollarSign, ArrowRight, AlertTriangle, CheckCircle } from 'lucide-react';
...
export default function WithdrawalPanel() {
    ...
    return (
        <div className="glass-panel p-6 rounded-xl border border-white/10">
            ...
            <form onSubmit={handleWithdraw} className="space-y-4">
                <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                        Note: Amounts &ge; $10,000 require Multi-Sig approval.
                    </p>
                </div>
            ...
            {result && (
                <div className={`mt-4 p-4 rounded-lg border flex items-start gap-3 ${result.status === 'REQUIRES_APPROVAL'
                        ? 'bg-yellow-500/10 border-yellow-500/30 text-yellow-400'
                        : result.status === 'COMPLETED'
                            ? 'bg-green-500/10 border-green-500/30 text-green-400'
                            : 'bg-red-500/10 border-red-500/30 text-red-400'
                    }`}>
                    {result.status === 'REQUIRES_APPROVAL' ? <AlertTriangle className="h-5 w-5 shrink-0" /> :
                        result.status === 'COMPLETED' ? <CheckCircle className="h-5 w-5 shrink-0" /> :
                            <AlertTriangle className="h-5 w-5 shrink-0" />}
                    <div>
                        <div className="font-bold text-sm">{result.status === 'REQUIRES_APPROVAL' ? 'Approval Required' : result.status}</div>
