...
import { ArrowRightLeft, ExternalLink, RefreshCw, ShieldCheck, Snowflake } from 'lucide-react';
...
export default function ColdStorageVault() {
  ...
  const fetchWallets = useCallback(async () => {
    try {
      const { data, error } = await supabase.from('cold_wallets').select('*').order('balance', { ascending: false });
    ...
    <div className="glass-panel p-6 rounded-xl border border-white/10 relative overflow-hidden">
      ...
      {showRebalance && (
        <div className="mb-6 p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-xl animate-in fade-in slide-in-from-top-2">
          ...
          <p className="text-xs text-gray-300 mb-3">Hot wallet liquidity is high ($2.45M). Recommended action:</p>
          <div className="flex items-center justify-between bg-black/40 p-3 rounded-lg border border-white/5">
            <div className="text-sm text-white">
              Move <span className="font-bold text-[#00FF94]">$500,000 USDT</span> to Vault Alpha
            </div>
      ...
      <div className="space-y-3 relative z-10">
        {isLoading ? (
          ...
          wallets.map((wallet) => (
            <div
              key={wallet.id}
              className="group p-4 bg-white/5 hover:bg-white/10 rounded-lg border border-white/5 hover:border-blue-500/30 transition-all"
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <span
                    className={`w-2 h-2 rounded-full ${wallet.status === 'ACTIVE' ? 'bg-green-500' : 'bg-red-500'}`}
                  />
