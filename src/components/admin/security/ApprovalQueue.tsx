            ...
            <div className="space-y-3">
                {requests.length === 0 ? (
                    ...
                    requests.map((req) => {
                        ...
                        return (
                            <div key={req.id} className="flex items-center justify-between p-4 bg-white/5 rounded-lg border border-white/5 hover:border-[#F59E0B]/30 transition-colors group">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <div className="font-medium text-white flex items-center gap-2">
                                            {req.action_type}
                                            {isRequester && <span className="text-[10px] bg-gray-700 px-1.5 py-0.5 rounded text-gray-300">YOU</span>}
                                        </div>
                                        <div className="text-xs text-gray-500">
                                            Requested by <span className="text-gray-300">{req.requester.full_name || req.requester.email}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <button
                                        ...
                                        className={`px-4 py-2 rounded-lg text-sm font-bold transition-colors flex items-center gap-2 ${hasApproved
