    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Real-time oversight of all trading operations</p>
        </div>
        ...
        {(['ALL', 'FILLED', 'PENDING', 'REJECTED'] as const).map((status) => (
          ...
          >
            ...
            <Badge variant="secondary" className="ml-2">
              {status === 'ALL' ? trades.length : trades.filter((t) => t.status === status).length}
            </Badge>
          </Button>
        ))}
      </div>
      ...
      <Card>
        ...
        <CardContent>
          {filteredTrades.length === 0 ? (
            <p className="text-center text-muted-foreground py-8">No trading activity found</p>
          ) : (
                ...
                <tbody>
                  {filteredTrades.map((trade) => (
                    <tr key={trade.id} className="border-b hover:bg-muted/50">
                      <td className="p-2 font-mono text-xs">{new Date(trade.created_at).toLocaleString()}</td>
                      <td className="p-2 font-mono text-xs">{trade.user_id.substring(0, 8)}...</td>
                      ...
                      <td className="p-2 text-right font-mono">{trade.quantity.toFixed(4)}</td>
