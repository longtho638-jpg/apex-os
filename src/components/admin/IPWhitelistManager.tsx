...
import { AlertCircle, CheckCircle2, Globe, Loader2, Plus, Shield, Trash2 } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Restrict admin access to specific IP addresses</p>
        </div>
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg font-medium transition-colors ${
            enabled ? 'bg-emerald-600 hover:bg-emerald-500 text-white' : 'bg-white/10 hover:bg-white/20 text-gray-300'
          }`}
        >
      ...
      {error && (
        <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-lg flex items-center gap-2 text-red-400">
          ...
          {error}
          <button onClick={() => setError(null)} className="ml-auto text-xs">
            ✕
          </button>
        </div>
      )}
      ...
      {/* Add IP Form */}
      <div className="bg-[#0a0a0a] border border-white/10 rounded-lg p-4">
        <label className="block text-sm font-medium text-gray-400 mb-2">Add IP Address (supports CIDR notation)</label>
        ...
        <p className="text-xs text-gray-500 mt-2">Use CIDR notation (e.g., 192.168.1.0/24) to whitelist IP ranges</p>
      </div>

      {/* IP List */}
      <div className="bg-[#0a0a0a] border border-white/10 rounded-lg">
        <div className="p-4 border-b border-white/10">
          <h3 className="text-sm font-medium text-white">Whitelisted IPs ({allowedIPs.length})</h3>
        </div>
        ...
        {allowedIPs.length === 0 ? (
          <div className="p-8 text-center text-gray-500">
            ...
            <p className="text-xs mt-1 opacity-70">Add your current IP to get started</p>
          </div>
        ) : (
            ...
            {allowedIPs.map((ip, index) => (
              <div key={index} className="p-4 flex items-center justify-between hover:bg-white/5 transition-colors">
                <div>
                  ...
                  {ip === currentIP && <span className="ml-2 text-xs text-emerald-500">(current)</span>}
                </div>
                <button
