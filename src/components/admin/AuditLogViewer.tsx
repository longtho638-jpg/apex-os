...
export default function AuditLogViewer() {
  const _t = useTranslations('AdminAuditLogs'); // Assuming translations exist, fallback to English keys if not
  ...
  }, [token, fetchLogs]); // Re-fetch when page, filters, or token changes
              ...
              {loading ? (
                ...
                logs.map((log) => (
                  <tr key={log.id} className="hover:bg-[#1E293B]/50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-white font-medium">{log.admin_users?.email || 'Unknown'}</td>
