...
import { ArrowDown, ArrowUp, Activity, DollarSign, TrendingUp, BarChart3 } from "lucide-react";
import { useState, useEffect } from "react";
import { fetchWatchlist, fetchPositions, fetchSystemStatus } from "@/lib/api";
        ...
        <div className="flex h-full w-full flex-col bg-[#050505] text-white font-mono">
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className="w-64 border-r border-[#262626] bg-[#0a0a0a] flex flex-col">
                    <div className="p-3 text-xs font-bold text-gray-500 uppercase tracking-wider border-b border-[#262626]">Watchlist</div>
                    <div className="flex-1 overflow-y-auto">
                        {watchlist.map((item) => (
                            <div key={item.symbol} className="flex items-center justify-between p-3 hover:bg-[#1a1a1a] cursor-pointer border-b border-[#1a1a1a] transition-colors">
                                <div>
                                ...
                                <div className="text-right">
                                    ...
                                    <div className={cn("text-xs flex items-center justify-end gap-1", item.change >= 0 ? "text-green-500" : "text-red-500")}>
                        ...
                        <div className="h-full w-full flex items-end justify-between gap-1 opacity-80">
                            ...
                            {Array.from({ length: 40 }).map((_, i) => {
                                ...
                                return (
                                    <div
                                        key={i}
                                        className={cn("w-full rounded-sm opacity-80 hover:opacity-100 transition-opacity", isGreen ? "bg-green-500/20 border-t border-green-500" : "bg-red-500/20 border-t border-red-500")}
                                        style={{ height: `${height}%` }}
                                    />
                                )
                            })}
                        ...
                        <div className="flex items-center gap-4 border-b border-[#262626] px-4">
                            {['POSITIONS', 'ORDERS', 'HISTORY'].map(tab => (
                                <button
                                    ...
                                    className={cn("h-10 text-xs font-bold border-b-2 px-2 transition-colors", activeTab === tab.toLowerCase() ? "border-blue-500 text-white" : "border-transparent text-gray-500 hover:text-gray-300")}
                                >
                                    {tab}
                                </button>
                            ))}
                                ...
                                <tbody>
                                    {positions.map(pos => (
                                        <tr key={pos.symbol} className="border-b border-[#1a1a1a] hover:bg-[#1a1a1a]">
                                            ...
                                            <td className={cn("p-3 text-right font-medium", pos.pnl >= 0 ? "text-green-500" : "text-red-500")}>
                                                {pos.pnl >= 0 ? "+" : ""}{pos.pnl.toFixed(2)} ({pos.pnl_percent}%)
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
                ...
                <div className="w-72 border-l border-[#262626] bg-[#0a0a0a] p-4 flex flex-col gap-4">
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input type="text" value="AAPL" className="w-full bg-[#1a1a1a] border border-[#333] rounded p-2 text-sm focus:border-blue-500 outline-none" readOnly />
                        </div>

                        <div className="grid grid-cols-2 gap-2">
                            <div>
                                <label className="text-xs text-gray-500 block mb-1">Quantity</label>
                                <input type="number" defaultValue="10" className="w-full bg-[#1a1a1a] border border-[#333] rounded p-2 text-sm focus:border-blue-500 outline-none" />
                            </div>
                            <div>
                                <label className="text-xs text-gray-500 block mb-1">Price</label>
                                <input type="number" defaultValue="173.50" className="w-full bg-[#1a1a1a] border border-[#333] rounded p-2 text-sm focus:border-blue-500 outline-none" />
                            </div>
                        </div>

                        <div className="grid grid-cols-2 gap-2 pt-4">
                            <button className="bg-green-600 hover:bg-green-500 text-white font-bold py-3 rounded transition-colors">BUY</button>
                            <button className="bg-red-600 hover:bg-red-500 text-white font-bold py-3 rounded transition-colors">SELL</button>
                        </div>
                    </div>
