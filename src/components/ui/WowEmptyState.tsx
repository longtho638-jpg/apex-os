...
export function WowEmptyState({ title, description, icon: Icon, action, className }: WowEmptyStateProps) {
  return (
    <div
      className={cn(
        'relative overflow-hidden rounded-2xl border border-white/10 bg-black/20 p-12 text-center',
        className,
      )}
    >
      ...
      <div className="relative z-10 flex flex-col items-center justify-center space-y-6">
        ...
        {action && (
          ...
          >
            ...
            <span className="relative flex items-center gap-2">{action.label}</span>
          </motion.button>
        )}
      </div>
    </div>
