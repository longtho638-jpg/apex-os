...
export const ActionNode = memo(({ data }: { data: any }) => {
  ...
  return (
    <div className="relative group">
      ...
      <div
        className={`absolute -inset-1 bg-gradient-to-r ${glowColor} rounded-xl blur opacity-30 group-hover:opacity-80 transition duration-500 animate-pulse`}
      ></div>

      {/* Card Body */}
      <div
        className={`relative bg-black/90 backdrop-blur-xl border border-white/10 rounded-xl p-4 min-w-[140px] shadow-2xl flex flex-col items-center text-center transition-all hover:${isBuy ? 'border-emerald-500/50' : isSell ? 'border-red-500/50' : 'border-yellow-500/50'}`}
      >
        <Handle
          type="target"
