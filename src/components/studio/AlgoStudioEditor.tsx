...
import { runBacktest, generateMarketData, BacktestResult, optimizeStrategy, OptimizationResult } from '@/lib/algo/backtest-engine';
import { Play, Save, Plus, MousePointer2, Box, ArrowRightCircle, Zap, Activity, Minus, Maximize, Wand2 } from 'lucide-react';
...
const initialNodes: AlgoNode[] = [
  { id: '1', type: 'indicator', position: { x: 250, y: 100 }, data: { label: 'RSI', params: { period: 14, source: 'close' } } },
...
function CustomControls() {
  ...
  return (
    <Panel position="bottom-right" className="bottom-24 right-6 bg-black/80 backdrop-blur-xl border border-white/10 p-1.5 rounded-xl flex flex-col gap-1 shadow-2xl shadow-black/50">
      <button onClick={() => zoomIn({ duration: 300 })} className="p-2 hover:bg-white/10 rounded-lg text-zinc-400 hover:text-emerald-400 transition group" title="Zoom In">
        <Plus className="w-4 h-4 group-active:scale-90 transition-transform" />
      </button>
      <button onClick={() => zoomOut({ duration: 300 })} className="p-2 hover:bg-white/10 rounded-lg text-zinc-400 hover:text-emerald-400 transition group" title="Zoom Out">
        <Minus className="w-4 h-4 group-active:scale-90 transition-transform" />
      </button>
      <div className="h-px w-full bg-white/10 my-0.5" />
      <button onClick={() => fitView({ duration: 300 })} className="p-2 hover:bg-white/10 rounded-lg text-zinc-400 hover:text-emerald-400 transition group" title="Fit View">
...
function AlgoStudioContent() {
  ...
  const onConnect = useCallback(
    (params: Connection) => setEdges((eds) => addEdge({ ...params, type: 'particle', animated: true, style: { stroke: '#34D399' } }, eds)),
    [setEdges],
  );
  ...
  const onDragOver = useCallback((event: React.DragEvent) => {
    ...
    event.dataTransfer.dropEffect = 'move';
      ...
      (event: React.DragEvent) => {
        ...
        // @ts-ignore - Typescript inference for setNodes union is too strict here
      ...
      <main className="flex-1 relative flex flex-col h-full">
        ...
        <header className="absolute top-0 left-0 right-0 h-16 flex items-center justify-between px-6 z-20 pointer-events-none">
            <div className="pointer-events-auto bg-black/60 backdrop-blur-md border border-white/10 p-2 rounded-xl flex items-center gap-4">
                <h1 className="font-bold text-lg flex items-center gap-2 tracking-tight px-2">
                    <span className="bg-gradient-to-r from-emerald-400 to-cyan-400 bg-clip-text text-transparent">Algo Studio</span> 
                </h1>
                <div className="flex gap-2">
                    <button onClick={handleOptimize} className="flex items-center gap-2 px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold rounded-lg transition shadow-[0_0_15px_rgba(147,51,234,0.4)] animate-pulse">
                        <Wand2 className="w-3 h-3" /> Auto-Optimize
                    </button>
                    <button onClick={handleCompile} className="flex items-center gap-2 px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-black text-xs font-bold rounded-lg transition shadow-[0_0_15px_rgba(16,185,129,0.4)]">
        ...
        <div className="flex-1 h-full" ref={reactFlowWrapper}>
            ...
            >
                ...
                <Panel position="top-left" className="top-20 left-4 bg-black/80 backdrop-blur-xl border border-white/10 p-4 rounded-2xl shadow-2xl w-64 pointer-events-auto">
