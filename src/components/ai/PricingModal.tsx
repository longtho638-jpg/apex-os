...
// Mock trackEvent for now
const trackEvent = (params: any) => {
    console.log('[Analytics Mock]', params);
};
...
export function PricingModal({ isOpen, onClose, userId, currentTier, trigger }: PricingModalProps) {
  ...
  if (showCheckout) {
    return (
      <CheckoutModal
        tier={selectedTier.toUpperCase() as 'FOUNDERS' | 'PREMIUM'} // Map to checkout tiers
                ...
                >
                  {config.highlight && (
                    <div className={`absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 ${
                        tier === 'pro' ? 'bg-emerald-500' : tier === 'trader' ? 'bg-blue-500' : 'bg-purple-500'
