...
export function PnLDashboard({ userId }: PnLDashboardProps) {
  ...
  const fetchPnLMetrics = useCallback(async () => {
    try {
      setLoading(true);
      ...
      const realizedPnL = closedPositions.reduce((sum: number, pos: TradingPosition) => {
        // Simplified - in production, calculate actual PnL from close price
        return sum + (pos.unrealized_pnl || 0);
      }, 0);

      // Calculate unrealized PnL from open positions
      const unrealizedPnL = openPositions.reduce((sum: number, pos: TradingPosition) => {
        return sum + (pos.unrealized_pnl || 0);
      }, 0);

      // Calculate exposure
      const exposure = openPositions.reduce((sum: number, pos: TradingPosition) => {
        return sum + pos.entry_price * pos.quantity * pos.leverage;
      }, 0);

      // Calculate win rate
      const profitableTrades = closedPositions.filter((pos: TradingPosition) => (pos.unrealized_pnl || 0) > 0).length;
      const lossTrades = closedPositions.filter((pos: TradingPosition) => (pos.unrealized_pnl || 0) <= 0).length;
  ...
  const totalReturn = metrics.accountBalance > 0 ? (totalPnL / metrics.accountBalance) * 100 : 0;
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Track your trading performance and profitability</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">From {metrics.totalTrades} closed trades</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div
              className={`text-2xl font-bold ${metrics.totalUnrealizedPnL >= 0 ? 'text-green-500' : 'text-red-500'}`}
            >
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <span className="font-mono font-bold text-lg">${metrics.accountBalance.toFixed(2)} USDT</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-muted-foreground">Total Exposure</span>
              <span className="font-mono font-bold">${metrics.totalExposure.toFixed(2)}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-muted-foreground">Exposure %</span>
              <span
                className={`font-bold ${
                  ((metrics.totalExposure / metrics.accountBalance) * 100) > 80 ? 'text-red-500' : 'text-green-500'
                }`}
              >
                {metrics.accountBalance > 0 ? ((metrics.totalExposure / metrics.accountBalance) * 100).toFixed(1) : 0}%
              </span>
            </div>
          </div>
        </CardContent>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-3xl font-bold text-green-500">{metrics.profitableTrades}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <p className="text-3xl font-bold text-red-500">{metrics.lossTrades}</p>
          </CardContent>
        </Card>
      </div>
    </div>
