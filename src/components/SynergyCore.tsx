  ...
  return (
    <div className="relative w-full h-[600px] bg-black/50 rounded-3xl border border-[#00FF00]/20 overflow-hidden backdrop-blur-md">
      ...
      <div className="absolute top-6 right-6 text-right z-10">
        ...
        <div className="text-3xl font-bold text-[#00FF00]">{synergyData.activeUsers.toLocaleString()}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="relative">
      ...
      <svg width="300" height="300" className="relative z-10">
        ...
        {/* Background circle */}
        <circle cx="150" cy="150" r="120" fill="none" stroke="rgba(0,255,0,0.1)" strokeWidth="8" />
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <div className="text-center">
          ...
          <motion.div className="text-4xl font-bold text-[#00FF00] mb-1" key={Math.floor(data.totalVolume)}>
            ${(data.totalVolume / 1000000).toFixed(2)}M
          </motion.div>
          <div className="text-xs text-gray-500">Next Tier: {data.tierName}</div>
          <div className="text-xs text-[#00FF00] mt-1">{data.progress.toFixed(1)}% completed</div>
        </div>
      </div>
    </div>
  );
...
function ToastNotification({ toast }: { toast: Toast }) {
  ...
  const icon = toast.type === 'whale' ? '🐋' : toast.type === 'partner' ? '🤝' : '🎉';
