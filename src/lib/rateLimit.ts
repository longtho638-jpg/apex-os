...
export async function checkRateLimit(
  key: string,
  config: RateLimitConfig = LIMITS.API_STANDARD,
): Promise<{
  success: boolean;
  limit: number;
  remaining: number;
  ...
  try {
    ...
    const { data: record, error } = await supabase.from('rate_limits').select('*').eq('key', key).single();
    ...
    if (record.window_start < windowStart) {
      // Reset window
      await supabase.from('rate_limits').update({ count: 1, window_start: now }).eq('key', key);
