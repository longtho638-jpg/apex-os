...
export function testTechnicalCalculator() {
    console.log('🧪 Testing Technical Calculator...\n');
    ...
    try {
        ...
        console.log(`  - ${prices.length} price points`);
...
export function testPortfolioReturns() {
    ...
    console.log('✅ Generated test trades:');
    console.log(`  - Total trades: ${trades.length}`);
    console.log(`  - Win rate: ${(trades.filter(t => t.pnlPercent > 0).length / trades.length * 100).toFixed(1)}%`);
    console.log(`  - Avg return: ${(returns.reduce((a, b) => a + b, 0) / returns.length).toFixed(2)}%`);
    console.log(`  - Best trade: +${Math.max(...returns).toFixed(2)}%`);
    console.log(`  - Worst trade: ${Math.min(...returns).toFixed(2)}%`);
...
if (typeof window !== 'undefined') {
    console.log('📊 Trade Data Test Utilities Loaded!\n');
    console.log('Available functions:');
    console.log('  - generateFakeTrades(count)');
    console.log('  - generateFakePriceHistory(days)');
    console.log('  - generateFakeCandles(count)');
    console.log('  - testTechnicalCalculator()');
    console.log('  - testPortfolioReturns()');
