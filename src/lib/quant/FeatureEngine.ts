...
export class QuantFeatureEngine {
  ...
  private detectMarketRegime(closes: number[], _volumes: number[]): MarketRegime {
  ...
  private skewness(values: number[]): number {
    ...
    const m3 = values.reduce((sum, val) => sum + ((val - mean) / std) ** 3, 0) / n;
  ...
  private kurtosis(values: number[]): number {
    ...
    const n = values.length;
    const m4 = values.reduce((sum, val) => sum + ((val - mean) / std) ** 4, 0) / n;
