...
export class TechnicalCalculator {
  ...
  static calculateMACD(prices: number[], fastPeriod: number = 12, slowPeriod: number = 26, signalPeriod: number = 9) {
    ...
    const fastEMA = TechnicalCalculator.calculateEMA(prices, fastPeriod);
    const slowEMA = TechnicalCalculator.calculateEMA(prices, slowPeriod);
    ...
    const _macdHistory = [macdValue]; // Simplified - should maintain history
  ...
  static calculateBollingerBands(prices: number[], period: number = 20, stdDev: number = 2) {
    ...
    const squaredDiffs = recentPrices.map((price) => (price - sma) ** 2);
   ...
   */
  static calculateAll(prices: number[], high: number[], low: number[], _volume: number[]) {
    const currentPrice = prices[prices.length - 1];

    return {
      price: currentPrice,
      ema9: TechnicalCalculator.calculateEMA(prices, 9),
      ema20: TechnicalCalculator.calculateEMA(prices, 20),
      ema50: TechnicalCalculator.calculateEMA(prices, 50),
      ema200: TechnicalCalculator.calculateEMA(prices, 200),
      rsi: TechnicalCalculator.calculateRSI(prices, 14),
      macd: TechnicalCalculator.calculateMACD(prices),
      atr14: TechnicalCalculator.calculateATR(high, low, prices, 14),
      bollingerBands: TechnicalCalculator.calculateBollingerBands(prices),
