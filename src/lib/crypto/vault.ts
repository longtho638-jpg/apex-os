...
export function encrypt(plaintext: string): string {
  try {
    ...
    return [salt.toString('base64'), iv.toString('base64'), authTag.toString('base64'), encrypted].join(':');
  } catch (error) {
...
export function verifyHash(value: string, hashedValue: string): boolean {
  ...
  return crypto.timingSafeEqual(Buffer.from(valueHash), Buffer.from(hashedValue));
}
