...
import type { PaymentMethod, Transaction, Wallet, Withdrawal } from '@/types/finance';
...
export class WalletService {
   ...
   */
  static async getTransactions(_userId: string, walletId: string, limit = 20, offset = 0): Promise<Transaction[]> {
    const { data, error } = await supabaseAdmin
      .from('transactions')
      .select('*')
  ...
  static async requestWithdrawal(userId: string, amount: number, paymentMethodId: string): Promise<Withdrawal> {
    ...
    // 2. Get Wallet
    const wallet = await WalletService.getWallet(userId, 'USDT');
