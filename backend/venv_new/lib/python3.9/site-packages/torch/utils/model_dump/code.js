...
class Hider extends Component {
  ...
  render({name, children}, {shown}) {
    const my_caret = html`<span class=caret onClick=${() => this.click()} >${caret(shown)}</span>`;
...
function ModelSizeSection({model: {file_size, zip_files}}) {
  ...
  const zip_overhead = file_size - store_size - compr_size;
...
class OpenTensorMemorySection extends Component {
  render({model: {model_data, constants}}) {
    const sizes = getTensorMemoryByDevice(new Map([
      ["data", model_data],
      ["constants", constants],
    ]));
    return html`
  ...
  render({model: {interned_strings}}, {blame_info}) {
    ...
    if (blame_info) {
      ...
      if (s_start != 0 || s_end != s_text.length) {
        const prefix = s_text.slice(0, s_start);
        const main = s_text.slice(s_start, s_end);
        const suffix = s_text.slice(s_end);
