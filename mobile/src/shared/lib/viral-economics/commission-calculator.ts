...
export async function calculateUserCommission(userId: string, month: string): Promise<UserCommission> {
  ...
  if (network) {
    for (const ref of network) {
      // Revenue generated by this referee
      // WARNING: This uses volume * avg_rate estimation. 
      // It does NOT account for zero-fee pairs or actual fee variations.
      // Realtime commission (processTradeCommission) is more accurate as it uses actual fees.
      const revenueGenerated = Number(ref.referee_volume) * EXCHANGE_AVG_REBATE_RATE;

      // Get rate for this specific level based on user's tier
      const userTier = (tierData.tier || 'FREE').toUpperCase() as TierId;
